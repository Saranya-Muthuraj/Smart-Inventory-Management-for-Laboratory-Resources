...
@st.cache_data
def load_data():
    return pd.read_csv("lab_usage1.csv", parse_dates=["Date"])
...
if expiring_soon.empty:
    ...
    df_display = expiring_soon[["Resource", "ExpiryDate"]].sort_values("ExpiryDate").copy()
    df_display["ExpiryDate"] = df_display["ExpiryDate"].dt.strftime('%Y-%m-%d')
